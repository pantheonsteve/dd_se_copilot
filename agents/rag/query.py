"""RAG query interface for any configured agent.
...
Usage:
    python query.py --agent librarian "What's the retention policy for log archives?"
    python query.py --agent value "What business outcomes has Datadog driven?"
    python query.py --agent librarian --interactive
    python query.py --agent librarian "question" --llm openai
"""
...
QUERY_REWRITE_PROMPT = agent["query_rewrite_prompt"].strip()
...
def _build_where(category: str | None = None, ticker: str | None = None) -> dict | None:
    """Build a ChromaDB where filter combining category and ticker metadata."""
    conditions = []
    if category:
...
def retrieve(collection, query_embedding: list[float], top_k: int = TOP_K,
             category: str | None = None, ticker: str | None = None) -> dict:
    where_filter = _build_where(category, ticker)
...
def multi_retrieve(collection, embeddings: list[list[float]], top_k: int = TOP_K,
                   category: str | None = None, ticker: str | None = None,
                   max_per_source: int = MAX_CHUNKS_PER_SOURCE) -> dict:
    """Retrieve across multiple query embeddings, deduplicate, and enforce source diversity."""
    seen_docs = set()
    merged = {"documents": [[]], "metadatas": [[]], "distances": [[]]}

    for emb in embeddings:
        results = retrieve(collection, emb, top_k=top_k, category=category, ticker=ticker)
    ...
    if merged["documents"][0]:
        ...
        for entry in combined:
            src = entry[1].get("source", "unknown")
        ...
        merged["documents"][0] = [c[0] for c in diverse]
        merged["metadatas"][0] = [c[1] for c in diverse]
        merged["distances"][0] = [c[2] for c in diverse]
...
def answer_question(question: str, category: str | None = None, llm: str = "claude") -> None:
    ...
    try:
        ...
        print("Error: No knowledge base found. Run 'python ingest.py --agent ...' first.")
        sys.exit(1)

    if collection.count() == 0:
        print("Error: Knowledge base is empty. Add documents and run 'python ingest.py --agent ...'.")
        sys.exit(1)
...
def interactive_mode(category: str | None = None, llm: str = "claude") -> None:
    name = agent["name"]
    print(f"{name} (interactive mode, llm={llm}) — type 'quit' to exit\n")
    while True:
        try:
...
def main():
    name = agent["name"]
    parser = argparse.ArgumentParser(description=f"Ask the {name} agent a question.")
