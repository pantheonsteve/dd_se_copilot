"""Document ingestion pipeline for the Value agent.

Walks the knowledge_base/ directory, chunks text files, embeds them via
OpenAI, and upserts into ChromaDB. Can also fetch and ingest web pages by URL,
or crawl an entire URL prefix.

Usage:
    python ingest.py                                              # ingest everything
    python ingest.py --path knowledge_base/some_subset            # ingest a subset
    python ingest.py --url "https://datadoghq.com/blog/foo/"     # ingest a single page
    python ingest.py --crawl "https://datadoghq.com/blog/"       # crawl all child pages
    python ingest.py --crawl "..." --max-pages 200               # with page limit
"""
...
def _get_http_client() -> httpx.Client:
    ...
    if _HTTP_CLIENT is None:
        _HTTP_CLIENT = httpx.Client(
            ...
            timeout=30,
            headers={"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) ValueBot/1.0"},
        )
    return _HTTP_CLIENT
...
def crawl_urls(base_url: str, max_pages: int = 0, delay: float = 0.5) -> list[dict]:
    ...
    prefix = f"{parsed_base.scheme}://{parsed_base.hostname}{parsed_base.path}"
...
def main():
    parser = argparse.ArgumentParser(description="Ingest documents into the Value knowledge base.")
    parser.add_argument(
        "--path",
        type=str,
        default=None,
        help="Path to ingest (default: entire knowledge_base/)",
    )
    parser.add_argument(
        "--url",
        type=str,
        default=None,
        help="URL of a single web page to fetch and ingest",
    )
    parser.add_argument(
        "--crawl",
        type=str,
        default=None,
        help="Base URL to crawl — fetches all child pages under this prefix",
    )
    parser.add_argument(
        "--max-pages",
        type=int,
        default=0,
        help="Maximum pages to crawl (default: 0 = no limit, crawl everything)",
    )
